...
{
    // BM6.2 - Báo cáo doanh thu theo khách hàng. Chọn tháng/năm → bấm báo cáo → đổ dữ liệu từ DB.
    public class DoanhThuTheoKhachHangViewModel : BaseViewModel
    {
        ...
        public DoanhThuTheoKhachHangViewModel() : this(new DialogService(), new DoanhThuRepository()) { }

        public DoanhThuTheoKhachHangViewModel(IDialogService dialog, DoanhThuRepository repo)
        {
            _dialog = dialog;
            _repo = repo;
        ...
        private void LapBaoCao()
        {
            try
            {
                KetQua.Clear();
                decimal tong = 0;
                foreach (var item in _repo.BaoCaoTheoKhachHang(Thang, Nam))
                {
                    KetQua.Add(item);
                    tong += item.DoanhThu;
                }
